...
SELECT target.thePosition, sum(abs(target.thePosition - x)) as fuel
FROM    bounds,
        pos,
        crabSequence as target
GROUP BY target.thePosition
    )
SELECT   thePosition, fuel as answer
FROM     targets
ORDER BY fuel
LIMIT    1;

# incorrect 451583, correct 336721
...
WITH pos AS (
    ...
    -- for each place they can move, find the fuel cost
    SELECT target.thePosition, abs(target.thePosition - x) as distance
    FROM    bounds,
            pos,
            crabSequence as target
), fuel AS (
    -- calculate fuel, n(n+1)/2 is formula for 1 + 2 + 3 ... + distance
    SELECT thePosition, (distance * (distance + 1) / 2) as fuel
    FROM   targets
)
SELECT   thePosition, sum(fuel) as answer
FROM     fuel
GROUP BY thePosition
ORDER BY answer
LIMIT    1;
